...
checkMemory noun verb xs
  -- if we found the result, return 100 * noun + verb
  | head result == 19690720 = Just (100 * noun + verb)
  | otherwise = Nothing
  where
...
main = do
  ...
  let (Just result) = head $ dropWhile isNothing [checkMemory x y originalContents | x <- [0..99], y <- [0..99]]
  print result
